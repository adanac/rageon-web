...
$(function() {
	...
	for (index = 0; index < numtags; ++index) {
		...
		if (classname.search(/\bnojs\b/, 0) != -1)
			Hide(thistag)

		if (classname.search(/\bonlyjs\b/, 0) != -1)
			Show(thistag)

			/* 59: middle class */

		if (classname.search(/\bmiddle\b/, 0) != -1) {
			S(thistag).marginTop = S(thistag).marginBottom = Px((H(thistag.parentNode) - H(thistag)) / 2)
		}

		/* 60: center class */

		if (classname.search(/\bcenter\b/, 0) != -1) {
			S(thistag).marginLeft = S(thistag).marginRight = Px((W(thistag.parentNode) - W(thistag)) / 2)
		}
		...
		if (classname.search(/\bresizeta\b/, 0) != -1 && tagname == 'textarea') {
		...
		if (classname.search(/\bdigitsonly\b/, 0) != -1) {
			thistag.onchange = thistag.onmouseout = thistag.onsubmit = function() {
		...
		if (classname.search(/\bplaceholder\b/, 0) != -1 && tagname == 'input') {
		...
		if (classname.search(/\bautofocus\b/, 0) != -1 && tagname == 'input'
		...
		while (repeat) {
			...
			if (thistag.className != oldclassname)
				repeat = true
			else
				repeat = false
		}
	...
	if (gfindex && window.opera)
		setTimeout(function()// Required by Opera
		{
			document.body.style += ""
		}, 1) // to redraw browser
	...
	if (cindex > 0) {
		...
		for (index = 0; index < cindex; ++index)
			html += InsVars('<a name=cite#1></a><li>#2</li>', index + 1,
					cites[index])
 ...
 * 
 * which replicates one of the superclasses built into this script. You can
 * override it, or add more superclasses with new names and values. You can also
 * add them to this file using the syntax shown above.
 * 
 ******************************************************************************/
